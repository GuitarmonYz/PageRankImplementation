...
ylabel("Run Time (sec)");
hold off
%% runtime on sparse matrix
figure;
for j = 1:5
    for i = 1:iter
        A = createSparse(iters(j), 0.3);
        tic;
        power_method(A, k);
        tPower(i) = toc;
        disp("power finished");
...
plot(iters, ttPower);
plot(iters, ttArnoldi);
title('runtime for power and arnoldi on real matrix');
legend("Power Method", "Arnoldi with QRS");
xlabel("matrix size");
ylabel("Run Time (sec)");
%% runtime on real data
figure;
data = load('../data/processed_data.mat');
subdata = data(1:10000, 1:10000);
subdata = full(subdata);
subdata(subdata >= 1e-5) = 1;
iters = [10, 100, 500, 700, 1000, 2000, 3000];
for j = 1:7
    for i = 1:iter
        A = subdata(1:iters(j), 1:iters(j));
        tic;
        power_method(A, k);
        tPower(i) = toc;
        disp("power finished");
